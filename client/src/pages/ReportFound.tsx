...
export const ReportFound: React.FC = () => {
  ...
  const [timeFound, setTimeFound] = useState<string>('');
  ...
  const validateStep1 = () => {
    setFormError(null);
    if (!title || !category || !description || !dateFound || !location) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post('/api/found', formData, {
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 md:p-10 shadow-sm relative overflow-hidden">
              ...
              {[1, 2, 3].map((num) => (
                <div
                  ...
                  className={`h-2 w-8 rounded-full transition-all ${
                    step >= num ? 'bg-teal-500' : 'bg-slate-100'
                  }`}
                />
              ))}
            </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              <div>
                <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Report Found Belonging</h2>
                <p className="text-slate-500 text-xs mt-1">Provide description metrics about where and what item you picked up on campus.</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    className="block w-full px-4 py-2.5 border border-slate-200 rounded-2xl text-slate-900 text-xs focus:ring-2 focus:ring-teal-500 focus:outline-none bg-slate-50/50"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="e.g. Cafeteria floor / Library Room 3"
                      className="block w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-2xl text-slate-900 text-xs focus:ring-2 focus:ring-teal-500 focus:outline-none bg-slate-50/50"
                    />
                  </div>
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Specify how you found it or where you left it for safekeeping (e.g. left with library receptionist)."
                    className="block w-full px-4 py-2.5 border border-slate-200 rounded-2xl text-slate-900 text-xs focus:ring-2 focus:ring-teal-500 focus:outline-none bg-slate-50/50"
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                    Identifying Questions / Verification details
                  </label>
                  <input
                    type="text"
                    value={identifyingDetails}
                    onChange={(e) => setIdentifyingDetails(e.target.value)}
                    placeholder="e.g. Describe the brand or wallpapers to verify ownership."
                    className="block w-full px-4 py-2.5 border border-slate-200 rounded-2xl text-slate-900 text-xs focus:ring-2 focus:ring-teal-500 focus:outline-none bg-slate-50/50"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end pt-4">
                <button
                  type="button"
                  onClick={handleNext}
                  className="bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3 px-6 rounded-2xl text-xs transition-all flex items-center gap-1.5"
                >
                  Continue
          ...
          {step === 2 && (
            ...
            >
              <div>
                ...
                <p className="text-slate-500 text-xs mt-1">Add reference photos showing the item (max 5).</p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 py-4">
                ...
                {files.length < 5 && (
                  <label className="h-24 rounded-2xl border-2 border-dashed border-slate-200 hover:border-teal-500 flex flex-col items-center justify-center text-slate-400 hover:text-teal-600 transition-all bg-slate-50/20 cursor-pointer">
          ...
          {step === 3 && (
            ...
            >
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-xs space-y-3">
                ...
                <div className="flex justify-between border-b border-slate-200/50 pb-2">
                  <span className="text-slate-400 font-medium">Location Found:</span>
                  <span className="text-slate-900 font-medium">📍 {location}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200/50 pb-2">
                  <span className="text-slate-400 font-medium">Date Found:</span>
                  <span className="text-slate-900 font-medium">{dateFound}</span>
                </div>
                {color && (
                  <div className="flex justify-between border-b border-slate-200/50 pb-2">
              ...
              <div className="flex justify-between pt-4 border-t border-slate-50">
                ...
                <button
                  ...
                  className="bg-teal-600 hover:bg-teal-700 text-white font-extrabold py-3.5 px-6 rounded-2xl text-xs transition-all flex items-center justify-center gap-1.5 shadow-md shadow-teal-500/25 disabled:opacity-50"
                >
                  {loading ? (
          ...
          {step === 4 && (
            ...
            >
              ...
              <div className="space-y-2">
                <h2 className="text-2xl font-black text-slate-900">Report Published!</h2>
                <p className="text-slate-500 text-xs leading-relaxed max-w-sm mx-auto">
                  Thank you. Your found item report has been published to the college feed. Classmates will contact you via chat to verify ownership details.
                </p>
              </div>
            </motion.div>
          )}
